...
PARALLEL_WORKERS       = int(os.getenv("PARALLEL_WORKERS", "1"))  # ihtiyaca göre arttır
TRIM_VARCHAR_OVERFLOW = os.getenv("TRIM_VARCHAR_OVERFLOW", "0") == "0"

def _print_full_df(df: pd.DataFrame, head: str = None):
    if head:
        print("\n" + "="*len(head))
        print(head)
        print("="*len(head))
    ...
    with pd.option_context(
        ...
        "display.width", 0,               # terminal genişliğine sığdırma
        "display.max_colwidth", None      # hiçbir kolonu kesme
    ):
        print(df.to_string(index=False))

def _to_csv_full(df: pd.DataFrame, path: str):
    # Metin normalizasyonu (isteğe bağlı ama güvenli)
    df = _sanitize_text_columns(df.copy())
    df.to_csv(path, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_MINIMAL)
    print(f"\nCSV yazıldı: {path}")


# --- ekle: metinleri CSV için sanitize eden yardımcı
def _sanitize_text_columns(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    for col in df.columns:
        # object/string türlerini hedefle
        if pd.api.types.is_object_dtype(df[col]) or pd.api.types.is_string_dtype(df[col]):
            df[col] = df[col].map(
                lambda v: (
                    v.replace('\x00', '')    # NUL yasak
                     .replace('\r\n', '\n')  # CRLF -> LF
                     .replace('\r', '\n')    # CR -> LF
...
def _enforce_varchar_limits(df: pd.DataFrame, cols_meta: List[Dict[str, Any]], cmap: Dict[str, str]) -> pd.DataFrame:
    """
    cols_meta: MSSQL kolon metadatası (char_max_len burada)
    cmap: MSSQL kolon adı -> pg/df için kullanılan lowercase isim haritası
    """
    if df.empty or not TRIM_VARCHAR_OVERFLOW:
        return df

    for cm in cols_meta:
        t = str(cm.get("data_type", "")).lower()
        L = cm.get("char_max_len")
        # nvarchar(max)/varchar(max) -> -1: limit yok
        if t in ("varchar", "nvarchar", "char", "nchar") and L not in (None, -1):
            try:
                limit = int(L)
            except Exception:
                continue
            new_col = cmap.get(cm["name"], (cm["name"] or "").lower())
            if new_col in df.columns:
                # stringe çevir, limitten uzunları kes
                s = df[new_col]
                # Pandas NA'lara dokunma
                mask = s.notna()
                df.loc[mask, new_col] = s[mask].astype(str).str.slice(0, limit)
    return df
...
class MssqlIntrospector:
    ...
    def select_df(self, sql: str, params: Tuple = None) -> pd.DataFrame:
        """Arbitrary SELECT -> DataFrame (sütun adlarını cursor.description'dan alır)."""
        rows = None
        def _run():
            cur = self.cn.cursor()
        ...
        return self._execute_with_retry(lambda: _run())
    ...
    def fetch_df_by_date_range(self, schema:str, table:str, cols:List[str],
        ...
        """
        start_iso / end_iso beklenen format: 'YYYY-MM-DD HH:MM:SS'
        Kolonu KAST ETMİYORUZ (indeks bozmasın). Parametreleri style 120 ile datetime'a çeviriyoruz.
        fld063 özelinde: NUL temizle + satır sonlarını normalize et + 800 karaktere kes.
        """
        projected = []
        for c in cols:
            if c.lower() == "fld063":
                # NUL sil, CRLF/CR -> LF normalizasyonu, 800'e kes; alias'ı fld063 olsun
                expr = (
                    "LEFT("
                    "REPLACE(REPLACE(REPLACE(CAST([fld063] AS NVARCHAR(MAX)), CHAR(0), ''), "
        ...
        # Alias sayesinde kolon adı yine 'fld063' olduğundan DataFrame kolon sırası/isimleri aynı kalsın:
...
class PostgresExecutor:
    def __init__(self, host:str, database:str, user:str, password:str, port:int=5432):
        # paramları sakla ki reconnect edebilelim
    ...
    def _should_reconnect(self, exc: Exception) -> bool:
        ...
        # tipik kopma/yeniden başlama durumları
        needles = [
    ...
    # PostgresExecutor.copy_from_df içinde, to_csv öncesi şunu ekleyin:
    def copy_from_df(self, table:str, df:pd.DataFrame)->int:
        ...
        # newline davranışı için StringIO(newline="") kullan
        ...
        df.to_csv(
            ...
            header=False,
            na_rep="\\N",               # PG NULL
            quoting=csv.QUOTE_MINIMAL,  # \n içeren alanlar otomatik quote edilir
            escapechar='\\',            # gerektiğinde kaçış
            lineterminator='\n'         # tek tip satır sonu
        )
        buf.seek(0)
        ...
        sql = (
            ...
            f"FROM STDIN WITH (FORMAT csv, NULL '\\N', ESCAPE '\\', QUOTE '\"')"
        )
        _guard_sql(sql)
        ...
        logger.info(f"[COPY pid={os.getpid()}] {table}: rows={len(df)} time={t1-t0:.2f}s")
        return len(df)
...
def day_buckets_int(start_date:dt.date, end_date:dt.date)->List[Tuple[int,int]]:
    ...
    # son gün eklendikten sonra döngüden çıkar
        cur=nxt
    return out
...
def worker_datetime(args)->int:
    ...
    try:
        ...
        if not df.empty:
            ...
            # --- BURAYA EKLE ---
            df = _sanitize_text_columns(df)                  # CR/LF, NUL temizliği
            #df = _enforce_varchar_limits(df, cols_meta, cmap) # VARCHAR/CHAR limitine göre kes
...
class DataMigrator:
    ...
    def _prep_table(self, schema:str, table:str)\
        ...
        """CREATE TABLE IF NOT EXISTS + append (TRUNCATE YOK)"""
    ...
    def process_iso8583_special(self, schema:str, table:str, date_col:str,
                                cutoff_one_shot_end_iso:str,
                                daily_start_iso:str, daily_end_iso:str):
        """(1) [lower_bound, cutoff_one_shot_end) tek sefer, (2) [daily_start, daily_end) günlük"""
        table_lower, cols_meta, cmap, _ = self._prep_table(schema, table)

        # (2) Günlük: daily_start → daily_end (exclusive)
        start_date = dt.datetime.strptime(daily_start_iso, "%Y-%m-%d %H:%M:%S").date()
        end_date   = (dt.datetime.strptime(daily_end_iso,   "%Y-%m-%d %H:%M:%S") - dt.timedelta(seconds=1)).date()
        ...
        with get_context("fork").Pool(processes=PARALLEL_WORKERS) as pool:
            for ins in pool.imap_unordered(worker_datetime, args_list, chunksize=1):
                total += ins
        logger.info(f"[DONE-daily] public.{table_lower} appended={total:,}")
def debug_dump_iso_row(ms: MssqlIntrospector,
                       fldAbsTime: int,
                       fldSqlDateTime_str: str,
                       out_csv_path: str = "tblIso8583_debug.csv"):
    """
    Ör: fldAbsTime=3836406242208,
        fldSqlDateTime_str='2021-07-27 20:24:02.207'
    """

    # Datetime karşılaştırmasında milisaniyeyi korumak için style 121 kullanıyoruz.
    sql = """
    SELECT *
    FROM [dbo].[tblIso8583]
    WHERE [fldAbsTime] = %s
      AND [fldSqlDateTime] = CONVERT(datetime, %s, 121);
    """
    df = ms.select_df(sql, (fldAbsTime, fldSqlDateTime_str))

    _print_full_df(df, head="dbo.tblIso8583 debug result")
    _to_csv_full(df, out_csv_path)
    return df
...
def migrate_selected_tables():
    ...
    # ---- DEBUG: spesifik kaydı çek + yazdır + CSV'ye dök
    debug_dump_iso_row(
        ms,
        fldAbsTime=3836406242208,
        fldSqlDateTime_str="2021-07-27 20:24:02.207",
        out_csv_path="tblIso8583_debug.csv"
    )

    # İstersen burada return edip çıkabilirsin
    ms.close()
    return


# ================= ORCHESTRATION =============
def migrate_selected_tables_2():
    env=load_configs()
    ms_server = env["mssql"]["host"] if env["mssql"].get("port") in (None,"",0) else f"{env['mssql']['host']},{env['mssql']['port']}"
    ...
    """
    # ---- 1) tblgenkartsay — günlük (2008-03-01 dahil → bugün)
    try:
        logger.info(f"[RUN] dbo.tblgenkartsay (daily int, append)")
    ...
    # ---- 2) tblkklogstr_new — günlük (2017-01-01 dahil → 2025-01-01 hariç)
    try:
        pg.ensure_alive()
        logger.info(f"[RUN] dbo.tblkklogstr_new (daily int, append)")
        start = dt.date(2017,1,7)
        end_exclusive = dt.date(2025,1,1)
        # day_buckets_int [start, next) mantığında; end_exclusive-1'e kadar kapsamak için end = end_exclusive - 1 gün
    ...
    """
    # ---- 3) tblIso8583 — ( < 2021-01-01 ) tek sefer, sonra günlük 2021-07-01 → 2024-06-01 (exclusive)
    try:
        pg.ensure_alive()
        logger.info(f"[RUN] dbo.tblIso8583 (special append)")
        cutoff_bulk_end = "2021-01-01 00:00:00"  # one-shot üst sınır (exclusive)
        daily_start     = "2021-07-16 00:00:00"  # dahil
        daily_end       = "2022-01-01 00:00:00"  # 2024-05 sonuna kadar (exclusive)
        mig.process_iso8583_special("dbo","tblIso8583","fldSqlDateTime",
                                    cutoff_one_shot_end_iso=cutoff_bulk_end,
                                    daily_start_iso=daily_start,
                                    daily_end_iso=daily_end)
    except Exception as e:
