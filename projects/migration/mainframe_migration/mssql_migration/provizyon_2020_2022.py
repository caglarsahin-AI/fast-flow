...
PER_DAY_WORKERS = int(os.getenv("PER_DAY_WORKERS", "4"))  # aynı gün için eşzamanlı saatlik işçi sayısı (saatlik modda)
# ================== PARAMS ===================
...
class PostgresExecutor:
    ...
    def copy_from_df(self, table:str, df:pd.DataFrame)->int:
        ...
        sql = (
            ...
            f"FROM STDIN WITH (FORMAT csv, DELIMITER E'\\t', NULL '\\N', ESCAPE '\\', QUOTE '\"')"
        )
        _guard_sql(sql)
        def _run():
            with self.cn.cursor() as cur:
...
class DataMigrator:
    ...
    def process_datetime_daily_parallel(self, schema:str, table:str, date_col:str, buckets:List[Tuple[str,str]]):
        ...
        # küçük bir ANALYZE iyi olur
        try: self.pg.analyze(table_lower)
        except Exception as e: logger.warning(f"[ANALYZE WARN] {table_lower}: {e}")

# ================= ORCHESTRATION =============
ARCHIVE_TABLES = [
    "tblProvzArsivstr_2020_1","tblProvzArsivstr_2020_2","tblProvzArsivstr_2020_3","tblProvzArsivstr_2020_4",
    "tblProvzArsivstr_2021_1","tblProvzArsivstr_2021_2","tblProvzArsivstr_2021_3","tblProvzArsivstr_2021_4",
...
def migrate_provz_archives_daily():
    ...
    for tbl in ARCHIVE_TABLES:
        try:
            ...
            # 1) Önce tabloyu PG'de oluştur (introspect)
            #    process fonksiyonu zaten _prep_table çağırıyor; ama min/max için şimdiden kolonları doğruluyoruz
            mn, mx = ms.get_min_max_datetime(schema, tbl, date_col)
            if not mn or not mx:
                logger.warning(f"[SKIP] {schema}.{tbl} — {date_col} boş görünüyor (MIN/MAX yok).")
                continue

            # Günlük bucketlar (inclusive)
            start_day = mn.date()
            end_day   = mx.date()
